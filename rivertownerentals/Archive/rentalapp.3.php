...
`;
  ...
  // Array to store the number of days in each month. NOTE Feb = 29 leap year.
  ...
  $('.date').on('keyup', function(e) {
    ...
    if(inputValue.length === 1 && parseInt(inputValue.slice(0, 1)) > 1) {
      $(this).addClass('invalid');
      $(this).data('invalid', true);
      return;
    }
    if(inputValue.length === 2 && parseInt(inputValue.slice(0, 2)) > 12) {
      $(this).addClass('invalid');
      $(this).data('invalid', true);
      return;
    }
    ...
    if(inputValue.length >= 3) {
      let dayTens = parseInt(inputValue.slice(2, 3));
    
      if(dayTens > daysInMonth[month -1].slice(0, 1)) {
        $(this).addClass('invalid');
        $(this).data('invalid', true);
        return;
      }
    ...
    if(inputValue.length >= 4) {
      ...
      if(day > daysInMonth[month -1]) {
        ...
        // Clear invalid class if day is now valid
        $(this).removeClass('invalid');
        $(this).data('invalid', false);
      }
    }
    ...
    // Now we have added two /s so the full length is now 10

    if(inputValue.length === 10) {
      // Check if this year is before the current date or after the current year
      // These are the only 'name' values for dates

      const beforeAfter = { "ExpirationDate": "after", "SigDate": "current", "DateOfBirth": "before" };

      const propName = $(this).prop("name");
      const currDate = new Date().getFullYear();
      let startDate, endDate;

      switch(beforeAfter[propName]) {
        case "after":
          startDate = currDate;
          endDate = 2050;
          break;
  ...
  $('.phone').on('keyup', function(e) {
    ...
    if(inputValue.length >= 9) { // Adjusted this condition
  ...
  $('.phone, .ss, .date').on('blur', function(e) {
  ...
  $("form").on("submit", function(e) {
    let invalid = $('.phone, .date, .ss').filter(function() {
